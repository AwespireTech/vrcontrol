...
export type QuestActionExecuteTarget = {
  id: string
  label: string
  ip?: string
  status?: string
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-90">
      <div className="surface-card mx-4 max-h-[80vh] w-full max-w-2xl overflow-y-auto p-6">
        <h2 className="mb-4 text-2xl font-bold text-foreground">執行動作: {actionName}</h2>

        <div className="mb-4">
          <div className="mb-2 flex items-center justify-between">
            <p className="text-sm font-semibold text-foreground">
              選擇要執行的設備 ({selectedIds.length} 個已選)
            </p>
