...
export default function RoomMinimap({
  ...
  return (
    <section className={showDetails ? "surface-panel overflow-hidden p-5" : "console-control-panel__inner p-3"}>
      {showDetails ? (
      ...
      <div className={showDetails ? "grid gap-4 lg:grid-cols-[minmax(0,1fr)_13rem]" : ""}>
        <div className="relative overflow-hidden rounded-[12px] border border-border/60 bg-background/30 p-3">
          <div className="aspect-square w-full">
            {markers.length === 0 ? (
              <div className="flex h-full items-center justify-center rounded-[12px] border border-dashed border-border/50 bg-background/20 p-6 text-center text-sm text-foreground/55">
                目前沒有玩家座標資料。
              </div>
            ) : (
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="text-right text-[11px] text-foreground/50">
                          ...
                          <div>
                            {marker.isStale ? "stale" : marker.readyToMove ? "ready" : "tracking"}
                          </div>
                        </div>
                      </div>
