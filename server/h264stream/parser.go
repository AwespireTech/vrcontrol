...
type EmitFunc func(AccessUnit) error

func StreamAccessUnits(ctx context.Context, reader io.Reader, fps int, emit EmitFunc) error {
	if fps <= 0 {
		fps = 30
	}
	...
	for {
		...
		case <-statsTicker.C:
			if !firstPacketAt.IsZero() {
				log.Printf("[H264][stream] frames/s=%d", framesSent)
			}
			framesSent = 0
		default:
		}
		...
		if n > 0 {
			if firstDataAt.IsZero() {
				...
				log.Printf("[H264][stream] first source bytes after=%s", firstDataAt.Sub(streamStartedAt).Truncate(10*time.Millisecond))
			}
		...
		if err != nil {
			...
			if err == io.EOF {
				if len(accessUnitNALUs) > 0 {
					if _, _, flushErr := emitAccessUnit(emit, accessUnitNALUs, latestSPS, latestPPS, frameDuration, hasSentKeyframe); flushErr != nil {
						return flushErr
					}
				}
				return nil
		...
		if firstPacketAt.IsZero() && len(buffer) >= 128*1024 && findStartCode(buffer, 0) < 0 {
			if lastNoStartCodeWarn.IsZero() || time.Since(lastNoStartCodeWarn) >= 1*time.Second {
				log.Printf("[H264][stream] no Annex-B start code yet, buffered=%d bytes", len(buffer))
		...
		if !firstDataAt.IsZero() && !hasSentKeyframe && time.Since(firstDataAt) >= 2*time.Second {
			if lastKeyframeWaitWarn.IsZero() || time.Since(lastKeyframeWaitWarn) >= 2*time.Second {
				log.Printf(
					"[H264][stream] waiting for first keyframe source_age=%s stream_age=%s buffered=%d",
		...
		for {
			...
			if len(nalu) > 0 {
				...
				if nalType == 5 && firstIDRAt.IsZero() {
					firstIDRAt = time.Now()
					if firstDataAt.IsZero() {
						log.Printf("[H264][stream] first IDR detected after=%s", firstIDRAt.Sub(streamStartedAt).Truncate(10*time.Millisecond))
					} else {
						log.Printf(
							"[H264][stream] first IDR detected stream_after=%s source_after=%s",
				...
				if nalType == 9 {
					written, sentKeyframe, writeErr := emitAccessUnit(emit, accessUnitNALUs, latestSPS, latestPPS, frameDuration, hasSentKeyframe)
					if writeErr != nil {
						return writeErr
					}
					framesSent += written
					if firstPacketAt.IsZero() && written > 0 {
						...
						log.Printf(
							"[H264][stream] first sample sent stream_after=%s source_after=%s",
					...
					if sentKeyframe {
						if !hasSentKeyframe {
							keyframeAt := time.Now()
							log.Printf(
								"[H264][stream] first keyframe sample sent stream_after=%s source_after=%s",
...
func emitAccessUnit(emit EmitFunc, nalus [][]byte, latestSPS, latestPPS []byte, duration time.Duration, hasSentKeyframe bool) (int, bool, error) {
	if len(nalus) == 0 {
	...
	if err := emit(AccessUnit{Data: accessUnit, Duration: duration, IsKeyframe: containsIDR}); err != nil {
		return 0, hasSentKeyframe, err
	}
