...
// StartSocket 啟動 Socket Server
func (c *RoomController) StartSocket(ctx *gin.Context) {
	roomID := ctx.Param("id")

	port, err := c.roomService.StartSocketServer(roomID)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{
			"success": false,
			"error":   err.Error(),
...
// StopSocket 停止 Socket Server
func (c *RoomController) StopSocket(ctx *gin.Context) {
	roomID := ctx.Param("id")

	if err := c.roomService.StopSocketServer(roomID); err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{
			"success": false,
			"error":   err.Error(),
		})
...
// GetSocketInfo 獲取 Socket Server 資訊
func (c *RoomController) GetSocketInfo(ctx *gin.Context) {
	roomID := ctx.Param("id")

	info, err := c.roomService.GetSocketServerInfo(roomID)
	if err != nil {
		ctx.JSON(http.StatusNotFound, gin.H{
			"success": false,
			"error":   err.Error(),
...
func (c *RoomController) SyncParameters(ctx *gin.Context) {
	roomID := ctx.Param("id")

	if err := c.roomService.SyncParameters(roomID); err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{
			"success": false,
			"error":   err.Error(),
		})
